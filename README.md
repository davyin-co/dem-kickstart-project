# DEM kickstart project template
Provides a Composer template for a new Davyin DEM project

Use [Composer](https://getcomposer.org/) to get Drupal + Davyin DEM Kickstart.
...
```
composer create-project davyin/dem-kickstart-project dem
```

Done! Use `composer require ...` to download additional modules and themes. For example, if you want to install the
gin theme you'd change into the target directory and require `drupal/gin`:
...
Starting from DEM Kickstart 3, this project includes the
[Kickstart Demo](https://www.drupal.org/project/dem_kickstart_demo) recipe by default. The full demo is designed
specifically for evaluating Davyin DEM or DEM Kickstart and should only be installed on new Kickstart
...
```bash
cd dem
composer remove drupal/dem_kickstart_demo
```

## What does the template do?

* Drupal is installed in the `docroot` directory.
* Modules (packages of type `drupal-module`) are placed in `docroot/modules/contrib/`
* Theme (packages of type `drupal-theme`) are placed in `docroot/themes/contrib/`
...
* Creates the `docroot/sites/default/files` directory.
